...
bool_t wait_pps_sync(uint8_t eTbgenInstance)
{
	...
	prev_ts = ullTbgenGet10MSCounter(eTbgenInstance);

	/* wait for 1s to pass - align to time boundary */
	while (--retries && (prev_ts == (ts = ullTbgenGet10MSCounter(eTbgenInstance)))) {
		vTaskDelay(1);
	}

	if (retries) {
		uint64_t diff = ts - prev_ts;
		if (diff != TBGEN_1S) {
			PRINTF("\r\n******* Warning: PPS diff not 1sec : 0x%lx%08lx **********\r\n",
			       PRINT_64_HI(diff), PRINT_64_LO(diff));
		}
	}
		...
		switch (recv->msg.type)
		{
			case TIME_ACTION_TDD:
				...
				else
				{
					...
					l1c_lp_timer_program(recv->msg.action.tdd_action.output.lp_timer,
								recv->msg.action.tdd_action.time_offset,
								recv->msg.action.tdd_action.output.steps,
								recv->msg.action.tdd_action.output.steps_count,
								recv->msg.action.tdd_action.repetitive);
