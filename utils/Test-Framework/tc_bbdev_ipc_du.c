...
#define FECA_SD_INPUT_MAX_SZ  0x12000
...
#define FECA_CD_DEMUX_NUM FECA_SD_N
#define FECA_SD_N 4

#define MAX_SD_FECA_BLOCKS 8
...
struct sd_crc_sts_desc crc_desc[FECA_SD_N];
...
/* Pending Jobs */
struct pending_jobs_t pending_jobs[MAX_SD_FECA_BLOCKS];
struct pending_jobs_t pending_jobs_se[MAX_QUEUES_PER_CORE];
...
volatile uint32_t feca_cmd_size[BBDEV_IPC_MAX_CORES] __attribute__ ((section (".hif"))) __attribute__ ((aligned (64)));
volatile BaseType_t feca_sd_cmd_addr[FECA_SD_N] __attribute__ ((section (".hif"))) __attribute__ ((aligned (64)));
...
BaseType_t feca_axi_addr_le[FECA_SD_N];
...
void feca_job_dump(struct bbdev_ipc_dequeue_op *deq_op)
{
	...
	log_info("Feca Block ID =%x\n\r", local_feca_job.t_blk_id);
	log_info("Input Source Data Length =%x\n\r", s_len);
	log_info("Printing output data at address = %x\n\r", deq_op->out_addr);
...
bool_t irq_recv_msg_from_vspa(uint32_t ul_irq, void *pv_dev_data)
{
	...
	if (IN_32(&p_vspa_regs->ulVspaStatus) & E200_MBOX1_STATUS) {
		log_dbg("%s: VSPA : [%d] E200_MBOX1_STATUS, intr = %u\n\r", __func__, e_vspa_core, ul_irq );
		OUT_32(&p_vspa_regs->ulVspaStatus, E200_MBOX1_STATUS );
		i_status = true;
	}
	...
	if (IN_32(&p_vspa_regs->ulVspaStatus) & VSPA_MBOX0_STATUS) {
		...
		exGeulAviHostHandleMboxIrq(p_avi_handler, e_vspa_core,
				VSPA_MBOX_0, &mbox);
...
init_qdma_decode_desc(struct feca_res *f_res,
		__attribute__((unused)) int tb_id,
		__attribute__((unused)) int queue_index,
		int ch_dec, u8 core_id)
{
...
#if VSPA_ENABLE
	VspaRegs_t *p_vspa_regs = NULL;
	p_vspa_regs = (VspaRegs_t *)VSPA_INST_BASE_ADDR(vspa_core);
	/* Send feca addresses to VSPA */
	for (int k = 0; k < FECA_SD_N; k++) {
		log_info("Sending feca ddr = %x, feca id = %d to VSPA\n\r", feca_axi_addr_le[k], k);

	        log_info("Reading VSPA Status 0x%X\n\r", IN_32(&p_vspa_regs->ulHostMboxStatus));
		/* using Mailbox 1 */
		while ((IN_32(&p_vspa_regs->ulHostMboxStatus) & 0x2));
		send_msg_to_vspa(SWAP_32(feca_axi_addr_le[k]), k, 1, vspa_core);
	}
#endif
}
...
init_feca_channels(struct feca_res *f_res, int init_core)
{
	...
	for (i = 0; i < attr->num_queues; i++) {
		...
		if (attr->qattr[i].op_type == BBDEV_IPC_OP_LDPC_DEC) {
			...
			/* 4 feca blocks will be used by this queue */
			for (int k = 0; k < FECA_SD_N; k++) {
				ret = init_feca_sd(f_res, k,
						   input_circ_size);
				if (ret) {
					log_err("init_feca_sd failed for tb_id: %d\n", i);
					return -1;
				}
				feca_sd_cmd_addr[k] = SWAP_32((BaseType_t)feca_get_ch_axi_addr(f_res->sd_chain,
								feca_get_ch_id(f_res->sd_chain, CH_CMD, k)));
			}
		}
		 ...
		 int queue_index)
{
	...
	pending_job = &pending_jobs[deq_op->feca_blk_id];
...
//		log_info("SE checking block id = %d data %d\n\r", deq_op->feca_blk_id, pending_job->data_to_read);
...
{
	struct bbdev_ipc_dequeue_op *deq_op;
	int queue_index = 0, i, empty_pending_jobs, k = 0;

	while (1) {
		deq_op = pending_jobs_se[queue_index].deq_op;
		if (deq_op) {
			if (pending_jobs_se[queue_index].qdma_pending == 1)
				pending_jobs_se[queue_index].deq_op = NULL;

			if (deq_op->feca_job_type == FECA_JOB_SE)
                                process_se_pending(&pending_jobs[queue_index],
                                                         queue_index);
		...
		for (i = 0; i < MAX_QUEUES_PER_CORE; i++) {
			...
			if (pending_jobs_se[queue_index].deq_op)
				empty_pending_jobs++;
		}
	...
	k = 0;
	while (1) {
		deq_op = pending_jobs[k].deq_op;
		if (deq_op) {
			if (pending_jobs[k].qdma_pending == 1)
				pending_jobs[k].deq_op = NULL;

			if (deq_op->feca_job_type == FECA_JOB_SD)
				pending_jobs[k].process_sd_pending_fn(
					&pending_jobs[k], k);
		}
		k++;

		empty_pending_jobs = 0;
		for (i = 0; i < FECA_SD_N; i++) {
			/* Check if all pending jobs are NULL */
			if (pending_jobs[k].deq_op)
				empty_pending_jobs++;
		}

		if (empty_pending_jobs == FECA_SD_N)
			break;
	}
...
process_se_start(struct bbdev_ipc_dequeue_op *deq_op, int queue_index)
{
         ...
         */
        pending_job = &pending_jobs_se[queue_index];
        pending_job->deq_op = deq_op;
...
static int bbdev_ipc_test(void)
{
	...
	struct bbdev_ipc_dequeue_op *deq_op[10];
	struct bbdev_ipc_dequeue_op *deq_op_se;
...
#if VSPA_ENABLE
#if 0
	/* Checking for VSPA queue and core. In case of multiple VSPA queues, this code
	 * will not able to identify which VSPA queue is for PUSCH */
	for (i = 0; i < attr->num_queues; i++) {
		if (attr->qattr[i].op_type == BBDEV_IPC_OP_LA12XX_VSPA) {
			log_info("Found VSPA queue %d core id %d\n\r", i, attr->qattr[i].core_id);
			vspa_core = attr->qattr[i].core_id;
			break;
		}
	}
	if (vspa_core == -1) {
		log_err("No LX2-VSPA queue found\n\r");
		return -1;
	}
#else
	/* VSPA core for PUSCH processing */
	vspa_core = VSPA_CORE_PUSCH;
#endif
#endif
	for (i = 0; i < attr->num_queues; i++) {
	...
	do {
		...
		if (queues_op_type[queue_id] == BBDEV_IPC_OP_LDPC_DEC) {
		/* waiting untill all ops processed */
		for (int k = 0; k < FECA_SD_N; k ++) {
			if (pending_jobs[k].deq_op) {
				if (pending_jobs[k].deq_op->feca_job_type == FECA_JOB_SD) {
					ret = pending_jobs[k].process_sd_pending_fn(
						&pending_jobs[k], queue_index);
					if (!ret) {
						log_info("send out ............................index = %d id = %d\n\r", queue_index, queue_ids[queue_index]);
						send_and_free_op(pending_jobs[k].deq_op, queue_index,
							 queue_ids[queue_index], 0);
						pending_jobs[k].deq_op = NULL;
					}

				} else {
					log_info("Invalid op, something is wrong in programming\n\r");
					/*TODO cleanup, if any or remove if else */
				}
			}
		}
		...
		ret = bbdev_ipc_dequeue_ops(BBDEV_IPC_DEV_ID_0,
				queue_id, &deq_op[ret], FECA_SD_N);
#if 1
		if (ret) {
			log_info("got IPC op index = %d, id = %d, total ops = %d\n\r", queue_index, queue_id, ret);
		}
		for (int k = 0; k < ret; k++) {
			log_info("feca id = %x, k = %d\n", deq_op[k]->feca_blk_id, k);
		}
#endif
		count  = ret;
		for (int k = 0; k < count; k++) {
			...
			feca_job_dump(deq_op[k]);
#endif
			if (deq_op[k]->feca_blk_id >= FECA_SD_N ||
					pending_jobs[deq_op[k]->feca_blk_id].deq_op != NULL) {
				log_err("Invalid job received, feca id %d in progress\n\r", deq_op[k]->feca_blk_id);
				send_and_free_op(pending_jobs[k].deq_op, queue_index,
			...
			ret = 0;
			if (deq_op[k]->feca_job_type == FECA_JOB_SD) {
				uint32_t swaped_in_len = SWAP_32(deq_op[k]->in_len);
				if (use_feca_sd_single_qdma == 1 ||
				swaped_in_len < sd_input_circ_size) {
					ret = process_sd_start(deq_op[k], queue_index);
				}
			} else {
				log_info("Invalid op or invalid size\n\r");
				send_and_free_op(deq_op[k], queue_index, queue_id, -1);
				pending_jobs[deq_op[k]->feca_blk_id].deq_op = NULL;
				continue;
			}
			if (!ret) {
				log_info("to send.......................\n\r");
				send_and_free_op(deq_op[k], queue_index, queue_id, 0);
				pending_jobs[deq_op[k]->feca_blk_id].deq_op = NULL;
			}
			else
				pending_jobs[deq_op[k]->feca_blk_id].deq_op = deq_op[k];
		}
		} else if (queues_op_type[queue_id] == BBDEV_IPC_OP_LDPC_ENC) {
			deq_op_se = pending_jobs_se[queue_index].deq_op;
			if (deq_op_se) {
				if (deq_op_se->feca_job_type == FECA_JOB_SE) {
					 ret = process_se_pending(&pending_jobs_se[queue_index],
                                                         queue_index);
				} else {
					log_info("Invalid Job type for this queue\n\r");
					/* TODO cleanup or remove if else */
				}
				if (!ret) {
					send_and_free_op(deq_op_se, queue_index,
						         queue_ids[queue_index], 0);
					pending_jobs_se[queue_index].deq_op = NULL;
			...
			//queue_id = queue_ids[queue_index];
			ret = bbdev_ipc_dequeue_ops(BBDEV_IPC_DEV_ID_0,
				        queue_id, &deq_op_se, 1);
			if (ret < 1)
				continue;
#ifdef FECA_IPC_DEBUG
			log_info("#########################################\n");
			log_info("Packet received on Queue ID: %d\n", queue_id);
			feca_job_dump(deq_op_se);
#endif
			ret = 0;
			if (deq_op_se->feca_job_type == FECA_JOB_SE) {
				ret = process_se_start(deq_op_se, queue_index);
			}

			if (!ret) {
	                        send_and_free_op(deq_op_se, queue_index, queue_id, 0);
		                pending_jobs_se[queue_index].deq_op = NULL;
			}
			else
				pending_jobs_se[queue_index].deq_op = deq_op_se;

		} else {
			log_info("invalid optype for queue id = %d\n\r", queue_id);
		}
	} while (1);
