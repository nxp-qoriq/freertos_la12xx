...
void l1c_shell_string_to_value(const char *str, cli_component_t cli_comp, void *value, type_t type)
{
	...
	struct {
		...
		[CLI_CFG] = {
			...
			{ "interface1",     L1C_CONFIG_INTERFACE     },
...
extern void vL1CDemoStart(int limited_slot_no);
...
extern void vL1CDPDRate(config_sample_rate_e rate);
extern void vL1CDPDConfigAdvance(uint32_t cnt);
...
char l1c_help_buf[] __attribute__ ((section (".shared.data"))) = "\
	...
	\r\n        dump \
	\r\nl1c start [optional limited slot number]\
	\r\nl1c stop \
	\r\n \
	...
	\r\n        config \
	\r\n            dump \
	\r\n            advance <number of us configured to advance the TX allowed signal related to the configured TBGENx> \
	\r\n            repeat <times the Tx and SRx are repeated> \
	\r\n            rate \
	\r\n                dpd  <DPD runs at rate: 245.76/491.52> \
	\r\n                ifft <input samples rate 122.88/245.76/491.52/983.04> \
	\r\n                fft  <SRx out samples rate: 245.76/491.52> \
	\r\n                dac  <Tx DAC sample rate: 245.76/491.52> \
	\r\n                adc  <SRx ADC sample rate: 122.88/245.76/983.04/1966.08> \
	\r\n            signal_seg <buffer segment count for the signal> \
	\r\n            training_seg <buffer segment count for TX ref signal, in e200 will be 2x multiplied automatically for SRx > \
	\r\n \
...
static uint32_t prvL1CParseIntf(const char *pcCommandString, uint8_t uOffset, BaseType_t *pxParameterStringLength)
{
...
static inline bool param_is(const char *p, const char *cmd)
{
	return !strncmp(p, cmd, strlen(cmd));
}
...
static void prvL1CParseDPD( const char *pcCommandString, uint8_t uParamOffset )
{
	...
	switch (cmd_type)
	{
		case L1C_DPD_CONFIG:
			...
			pcP = FreeRTOS_CLIGetParameter(pcCommandString, uParamOffset + 3L, &xParameterStringLength);
			...
			else if (param_is(pcParameter, "dump"))
			{
				vL1CDPDCfgDump();
				break;
			}
			// all commands below must have the second parameter
			else if(pcP == NULL)
			{
				PRINTF("No parameters! Use 'l1c help' command to see all l1c commands\r\n");
				break;
			}
			else if (param_is(pcParameter, "advance"))
			{
				vL1CDPDConfigAdvance(strtoul(pcP, (char **)NULL, BASE_DEC));
				break;
			}
			else if (param_is(pcParameter, "repeat"))
			{
				vL1CDPDConfigRepeat(strtoul(pcP, (char **)NULL, BASE_DEC));
				break;
			}
			else if (param_is(pcParameter, "rate"))
			{
				// third mandatory parameter
				pcParameter = FreeRTOS_CLIGetParameter(pcCommandString,
									uParamOffset + 4L,
									&xParameterStringLength);
				if (!pcParameter)
				...
				else if (param_is(pcP, "dpd"))
				{
					vL1CDPDRate(sample_rate_str2enum(pcParameter));
					break;
				}
				else if (param_is(pcP, "ifft"))
				{
					vL1CDPDIfftRate(sample_rate_str2enum(pcParameter));
					break;
				}
				else if (param_is(pcP, "fft"))
				{
					vL1CDPDFftRate(sample_rate_str2enum(pcParameter));
					break;
				}
				else if (param_is(pcP, "dac"))
				{
					vL1CDPDDACRate(sample_rate_str2enum(pcParameter));
					break;
				}
				else if (param_is(pcP, "adc"))
				{
					vL1CDPDADCRate(sample_rate_str2enum(pcParameter));
					break;
				}
				break;
			}
			else if (param_is(pcParameter, "training_seg"))
			{

				vL1CDPDConfigTrainingSeg(strtoul(pcP, (char **)NULL, BASE_DEC));
				break;
			}
			else if (param_is(pcParameter, "signal_seg"))
			{

				vL1CDPDConfigSignalSeg(strtoul(pcP, (char **)NULL, BASE_DEC));
				break;
			}
// delete from here:
			...
			if (pcParameter == NULL) {
				...
				pCfgMsg->ifft_sample_rate = 0;
				pCfgMsg->fft_sample_rate = 0;
			...
			/* ensure backward compatibility until the long config command it's removed */
			pCfgMsg->ifft_sample_rate = pCfgMsg->dpd_sample_rate;
...
// up to here
		...
		case L1C_DPD_CONFIG_SRX: /* <dcs: ls0/ls1/hs> <if: 0/1> */
			vL1CDPDConfigSRx(prvL1CParseIntf(pcCommandString, uParamOffset + 2L, &xParameterStringLength));
			break;

		case L1C_DPD_CONFIG_TX: /* <dcs: ls0/ls1/hs> <if: 0/1> */
			vL1CDPDConfigTx(prvL1CParseIntf(pcCommandString, uParamOffset + 2L, &xParameterStringLength));
			break;
...
portBASE_TYPE prvL1CDemoCommand( char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString )
{
	...
	else if ( CHECK_COMMAND( pcParameter, "start" ) )
	{
		set_e200_run_mode_tdd();
