...
void qdma_memcpy(void *dst, void *src, uint32_t size)
{
	qdma_flag = 0;
	...
	{
		DmaUSFFillIssue((BaseType_t)dst + (BaseType_t)(size/(size_t)ncq * i),
						(BaseType_t)src + (BaseType_t)(size/(size_t)ncq * i),
						size/(size_t)ncq, i, 0 /* rbp */);
	}
...
void vL1CBench(bench_type_e uBenchType, mem_type_e uMemType1, mem_type_e uMemType2, u32 uSizeInBytes, direction_e uTransferDirection, u32 uRepeats)
{
	...
	switch (uBenchType)
	{
		...
		case QDMA_BENCH:
			...
			for (i = 0; i < uRepeats; i++)
			{
				qdma_memcpy((void *)dst, (void *) src, uSizeInBytes);
				delta += stop_pmc-start_pmc;
			}
			break;
		default:
