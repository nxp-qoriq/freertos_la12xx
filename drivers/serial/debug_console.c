...
static void prvSendDataPolling(void *base, const uint8_t *txBuff,
				 uint32_t txSize);
static void prvReceiveDataPolling(void *base, uint8_t *rxBuff,
				    uint32_t rxSize);
...
/*       return -1; */
    ...
    result = _doprint(NULL, debug_putc, -1, (char *)fmt_s, arg);
...
static int debug_getc(unsigned char *ch, void* stream)
{
    if (stream)
	    return -1;
...
void prvSendDataPolling(void *base, const uint8_t *txBuff, uint32_t txSize)
{
    vSerialWriteBlocking(base, txBuff, txSize);
}

void prvReceiveDataPolling(void *base, uint8_t *rxBuff, uint32_t rxSize)
{
