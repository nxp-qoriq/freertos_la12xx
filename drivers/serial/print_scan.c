...
static void fput_pad(int32_t c, int32_t curlen, int32_t field_width,
		     int32_t *count, PUTCHAR_FUNC func_ptr, void *farg,
		     int *max_count);
...
#else
static int n_putchar(PUTCHAR_FUNC func_ptr, int chacter, void *p,
		     int *max_count)
{
    int result = 0;
    if (*max_count)
